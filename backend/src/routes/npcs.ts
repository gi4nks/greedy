...
router.get('/', (req: Request, res: Response) => {
  ...
  let rows;
  if (adventure) rows = db.prepare('SELECT * FROM npcs WHERE adventure_id = ?').all(adventure as any);
...
router.put('/:id', (req: Request, res: Response) => {
  ...
  db.prepare('UPDATE npcs SET name = ?, role = ?, description = ?, tags = ?, adventure_id = ? WHERE id = ?').run(name, role, description, stringifyTags(tags), adventure_id || null, req.params.id as any);
  const row = db.prepare('SELECT * FROM npcs WHERE id = ?').get(req.params.id as any);
  row.tags = parseTags(row.tags);
  res.json(row);
});

router.delete('/:id', (req: Request, res: Response) => {
  db.prepare('DELETE FROM npcs WHERE id = ?').run(req.params.id as any);
