...
async function testCharacterCreation() {
  ...
  try {
    ...
    const [testNpc] = await db.insert(characters).values({
      campaignId: testCampaign.id,
      name: 'Test NPC',
      race: 'Elf',
      level: 5,
    ...
    campaignCharacters.forEach(char => {
      console.log(`   - ${char.name} (${char.characterType}): ${char.race} Level ${char.level}`);
    });
    
    // Clean up
    await db.delete(characters).where(eq(characters.campaignId, testCampaign.id));
