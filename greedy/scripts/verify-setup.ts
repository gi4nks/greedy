...
function checkDirectory(
  ...
  return {
    ...
    message: exists
      ...
      : `${required ? "❌" : "⚠️"} ${name} directory missing`,
  };
}
...
function checkPackageScript(scriptName: string): CheckResult {
  try {
    ...
    return {
      ...
      message: hasScript
        ? `✅ npm run ${scriptName} available`
        : `❌ npm run ${scriptName} missing`,
    };
  } catch (error) {
    return {
      name: `npm script: ${scriptName}`,
...
async function runVerification() {
  ...
  results.push(
    checkFile("app/campaigns/[id]/page.tsx", "Campaign detail page"),
  );
  ...
  results.push(
    checkFile("scripts/migrate-from-backend.ts", "Migration script", false),
  );
