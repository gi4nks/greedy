 ...
 * to the new Next.js greedy database
 */
...
async function migrateData() {
  ...
  try {
    ...
    const tables = oldDb
      ...
      .all() as { name: string }[];
    console.log(
      `📋 Found ${tables.length} tables in old database:`,
    ...
    for (const table of tables) {
      ...
      try {
        ...
        if (!newTableExists) {
          console.log(
            `⚠️  Table '${tableName}' doesn't exist in new database, skipping...`,
          );
          continue;
        }
        ...
        console.log(
          `📄 Migrating ${records.length} records from '${tableName}'...`,
        );
        ...
        newDb.transaction(() => {
          for (const record of records) {
            try {
              ...
              console.error(
                `    ❌ Failed to migrate record from ${tableName}:`,
                error,
              );
            }
          }
        ...
        console.log(
          `  ✅ Successfully migrated ${records.length} records from '${tableName}'`,
        );
      } catch (error) {
