...
async function updateRingOfInvisibility() {
  ...
  try {
    ...
    const article = db.prepare('SELECT * FROM wiki_articles WHERE title = ?').get('Ring of Invisibility (Magic Ring)') as WikiArticleRow | undefined;
    ...
    const response = await fetch(`http://localhost:3000/api/wiki/article/0?title=${encodeURIComponent(article.title)}&full=true`);
    ...
    if (fullContent.content) {
      ...
      const parsedData = WikiDataService.parseMagicItemData(fullContent.content);
      ...
      updateStmt.run(fullContent.content, JSON.stringify(parsedData), article.id);
