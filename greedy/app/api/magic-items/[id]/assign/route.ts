...
export async function POST(
  ...
  try {
    ...
    const body = (await request.json()) as AssignMagicItemPayload & {
      entityId?: number;
    };
    ...
    if (!body.entityType || !entityIds.length) {
      return NextResponse.json(
        { error: "entityType and at least one entityId are required" },
        { status: 400 },
      );
    }
    ...
    return NextResponse.json(
      { error: "Failed to assign magic item" },
      { status: 500 },
    );
  }
}
...
export async function DELETE(
  ...
  try {
    ...
    const entityType = searchParams.get(
      "entityType",
    ) as MagicItemAssignableEntity | null;
