...
function isAssignableEntityType(
  value: string | null,
): value is MagicItemAssignableEntity {
  if (!value) return false;
  return SUPPORTED_MAGIC_ITEM_ENTITY_TYPES.includes(
...
export async function GET(request: NextRequest) {
  try {
    ...
    const campaignId = campaignIdParam
      ? Number.parseInt(campaignIdParam, 10)
      : undefined;
    ...
    return NextResponse.json(
      { error: "Failed to fetch assignable entities" },
      { status: 500 },
    );
  }
}
