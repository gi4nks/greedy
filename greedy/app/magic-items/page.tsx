...
import { getMagicItemsWithAssignments, type MagicItemFilters } from '@/lib/actions/magicItems';
...
async function getFilterOptions(): Promise<FilterOptionsResult> {
  const [manualTypeRows, manualRarityRows, wikiItemsRaw, campaignRows] = await Promise.all([
    ...
    db.select({ id: campaigns.id, title: campaigns.title }).from(campaigns).orderBy(campaigns.title),
  ]);
  ...
  const wikiTypeRarityOptions = wikiItemsRaw
    ...
    .flatMap((data) => [
      { type: data.type as string | null, rarity: data.rarity as string | null }
    ]);

  const manualTypes = manualTypeRows.map((row) => row.value).filter((value): value is string => !!value);
  const manualRarities = manualRarityRows.map((row) => row.value).filter((value): value is string => !!value);
  const wikiTypes = wikiTypeRarityOptions.map((item) => item.type).filter((value): value is string => !!value);
  const wikiRarities = wikiTypeRarityOptions.map((item) => item.rarity).filter((value): value is string => !!value);

  const typeOptions = Array.from(new Set([...manualTypes, ...wikiTypes])).sort();
  const rarityOptions = Array.from(new Set([...manualRarities, ...wikiRarities])).sort();
...
function buildFilters(searchParams?: SearchParams): {
  ...
  const entityType = entityTypeParam && supportedEntityTypes.has(entityTypeParam as typeof SUPPORTED_MAGIC_ITEM_ENTITY_TYPES[number])
  ...
  const filters: MagicItemFilters = {
    ...
    entityType,
    campaignId: campaignIdParam ? Number.parseInt(campaignIdParam, 10) || undefined : undefined,
  };
...
export default async function MagicItemsPage({ searchParams }: MagicItemsPageProps) {
  ...
  const totalFiltersActive = [filters.search, filters.type, filters.rarity, filters.campaignId, filters.entityType !== 'all' ? filters.entityType : null].filter(Boolean).length;
