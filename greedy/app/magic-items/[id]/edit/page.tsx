...
function serializeProperties(value: unknown): string {
    ...
    try {
        ...
        console.warn('Failed to stringify magic item properties', error);
        return '';
    }
}
...
export default async function EditMagicItemPage({ params }: EditMagicItemPageProps) {
    ...
    if (item.source === 'wiki') {
        return (
            <>
                <DynamicBreadcrumb items={[{ label: "Campaigns", href: "/campaigns" }, { label: "Magic Items", href: "/magic-items" }, { label: item.name, href: `/magic-items/${itemId}` }, { label: "Edit" }]} />
                    ...
                    <Card className="max-w-2xl">
                        ...
                        <CardContent>
                            <p className="text-base-content/60 mb-4">
                                Wiki items are read-only. You can still assign this item to entities using the assignment management below.
                            </p>
                                        ...
                                        <div key={entityType} className="space-y-3">
                                            ...
                                            <div className="space-y-2">
                                                {assignments.slice(0, 3).map((assignment) => (
                                                    <div key={assignment.id} className="flex items-center justify-between gap-2 rounded-md border border-base-200 bg-base-100 p-3 text-sm">
                                                        <div className="flex-1 min-w-0">
                                                            <p className="font-medium text-base-content truncate">{assignment.entityName}</p>
                                                            {assignment.campaignTitle && (
                                                                <p className="text-xs text-base-content/60 truncate">{assignment.campaignTitle}</p>
                                                            )}
                                                        </div>
