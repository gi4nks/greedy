 ...
 */
export function detectWikiItemCategory(title: string, content?: string): WikiItemCategory {
  const titleLower = title.toLowerCase();
  const contentLower = content?.toLowerCase() || '';
  
  // Magic Items - rings, wands, potions, etc.
  if (titleLower.includes('ring of') || 
  ...
  if (titleLower.includes(' spell') ||
      ...
      titleLower.match(/\b(fireball|magic missile|cure|heal|charm|sleep|invisibility)\b/) &&
  ...
  // Character Classes
  if (titleLower.includes('fighter') ||
  ...
  if (titleLower.includes('armor') ||
      ...
      contentLower.includes('armor class') && contentLower.includes('protection')) {
    return 'armor';
  }

  // Locations
  if (titleLower.includes('city of') ||
  ...
  if (titleLower.match(/\([^)]*(?:npc|character|person|lord|king|queen|priest|merchant|guard)\)/i) ||
  ...
  if (titleLower.includes('artifact') ||
...
export async function getAssignableEntities(campaignId: number): Promise<{
  ...
  try {
    const [charactersRes, npcsRes, questsRes, sessionsRes, locationsRes] = await Promise.all([
