...
export interface DnD5eRace {
  ...
  source: string;
  page?: number;
  ability?: Array<{ str?: number; dex?: number; con?: number; int?: number; wis?: number; cha?: number; choose?: any }>;
  size: string;
  speed: number;
  entries?: any[];
  subraces?: Array<{ name: string; entries?: any[] }>;
}
...
export interface DnD5eClass {
  ...
  startingProficiencies?: {
    ...
    weapons?: string[];
    tools?: string[];
    skills?: Array<{ choose?: any; from: string[] }>;
  };
...
export class DnD5eToolsService {
   ...
   */
  static parseEntries(entries: any[]): string {
    ...
    return entries.map(entry => {
      if (typeof entry === 'string') {
        ...
        return entry.items.map((item: any) => `• ${typeof item === 'string' ? item : this.parseEntries([item])}`).join('\n');
      } else if (entry.type === 'table' && entry.rows) {
        // Simple table rendering
        return entry.rows.map((row: any[]) => row.join(' | ')).join('\n');
      }
      return '';
