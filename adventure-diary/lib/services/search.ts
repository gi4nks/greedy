...
export class SearchService {
  ...
  static async search(
    ...
    try {
      ...
      const applyFilters = (baseQuery: any, table: any, textFields: string[], entityType: string) => {
      ...
      const applySorting = (query: any, table: any) => {
        switch (sortBy) {
          case 'date_desc':
      ...
      if (!entityTypes || entityTypes.includes('campaign')) {
        ...
        campaignQuery = applyFilters(campaignQuery, campaigns, ['title', 'description'], 'campaign');
      ...
      if (!entityTypes || entityTypes.includes('adventure')) {
        ...
        adventureQuery = applyFilters(adventureQuery, adventures, ['title', 'description'], 'adventure');
      ...
      if (!entityTypes || entityTypes.includes('session')) {
        ...
        sessionQuery = applyFilters(sessionQuery, sessions, ['title', 'text'], 'session');
      ...
      if (!entityTypes || entityTypes.includes('character')) {
        ...
        characterQuery = applyFilters(characterQuery, characters, ['name', 'description', 'backstory', 'personalityTraits', 'ideals', 'bonds', 'flaws'], 'character');
      ...
      if (!entityTypes || entityTypes.includes('npc')) {
        ...
        npcQuery = applyFilters(npcQuery, npcs, ['name', 'description', 'role'], 'npc');
      ...
      if (!entityTypes || entityTypes.includes('location')) {
        ...
        locationQuery = applyFilters(locationQuery, locations, ['name', 'description'], 'location');
      ...
      if (!entityTypes || entityTypes.includes('quest')) {
        ...
        questQuery = applyFilters(questQuery, quests, ['title', 'description'], 'quest');
      ...
      if (!entityTypes || entityTypes.includes('magic_item')) {
        ...
        magicItemQuery = applyFilters(magicItemQuery, magicItems, ['name', 'description'], 'magic_item');
