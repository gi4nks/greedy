...
import { campaigns, locations, adventures, gameEditions, wikiArticleEntities, wikiArticles } from '@/lib/db/schema';
import { eq, and } from 'drizzle-orm';
import { Card, CardContent, CardHeader, CardTitle } from '@/components/ui/card';
import { Button } from '@/components/ui/button';
import { Badge } from '@/components/ui/badge';
import { Skeleton } from '@/components/ui/skeleton';
import { MapPin, Edit, ArrowLeft, Building, Mountain, Trees } from 'lucide-react';
...
async function getLocation(locationId: number) {
  ...
  // Get campaign info if adventure exists
  const campaign = location.adventure?.campaignId ? await db
    .select({
      id: campaigns.id,
    ...
    .where(eq(campaigns.id, location.adventure.campaignId))
    .limit(1)
...
export default async function LocationPage({ params }: LocationPageProps) {
  ...
  if (!location || location.campaign?.id !== campaignId) {
    notFound();
  }
    ...
    <div className="space-y-6">
      ...
      {location.notes && (
        <Card>
          ...
          <CardContent>
            <div className="prose prose-sm max-w-none">
              <p className="whitespace-pre-wrap font-mono text-sm bg-amber-50 p-4 rounded border border-amber-200">
                {location.notes}
              </p>
            </div>
          </CardContent>
      ...
      {(!location.description && !location.notes && (!location.wikiEntities || location.wikiEntities.length === 0)) && (
        <Card>
