...
export async function POST(
  ...
  try {
    ...
    const { characterIds, relationshipType = 'companion', notes = '' } = await request.json();
    ...
    const results = await db.insert(wikiArticleEntities).values(assignments).returning();
    ...
    return NextResponse.json({ error: 'Failed to assign wiki monster' }, { status: 500 });
  }
}