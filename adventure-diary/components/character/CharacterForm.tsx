...
import { Plus, X, Save, ArrowLeft, Edit, Trash2, ChevronDown, ChevronUp } from 'lucide-react';
...
export default function CharacterForm({ character, campaignId, adventureId, mode }: CharacterFormProps) {
  ...
  const [wikiSpells, setWikiSpells] = useState<Array<{
    ...
    id: spell.id,
    name: spell.title,
    level: spell.parsedData?.level || 0,
    school: spell.parsedData?.school || 'common',
    description: spell.description || '',  // Use description field (mapped from rawContent)
    isPrepared: Boolean(spell.relationshipData?.isPrepared),
    isKnown: Boolean(spell.relationshipData?.isKnown)
  })) || []);
  ...
  const [wikiMonsters, setWikiMonsters] = useState<Array<{
    ...
    id: monster.id,
    name: monster.title,
    type: monster.parsedData?.type || 'monster',
    challengeRating: monster.parsedData?.challengeRating || '',
  ...
  const [magicItems, setMagicItems] = useState<Array<{
    ...
    id: item.id,
    name: item.title,
    rarity: item.parsedData?.rarity || '',
    type: item.parsedData?.type || '',
  ...
  const addItem = () => {
    updateFormData('items', [...formData.items, { title: '', description: '' }]);
  };
                        ...
                        <div key={item.id} className="border border-purple-200 bg-purple-50 rounded-lg">
                          <div className="flex justify-between items-start mb-2">
                            ...
                            <Button
                              ...
                              className="text-red-500 hover:text-red-700 hover:bg-red-50 p-1 h-auto mr-3 mt-3"
                            >
                      ...
                      return (
                        <div key={spell.id} className="border border-blue-200 bg-blue-50 rounded-lg">
                          ...
                          {isExpanded && spell.description && (
                            <div className="px-3 pb-3">
                              <MarkdownContent content={spell.description} />
                            </div>
                          )}
                        </div>
                      );
