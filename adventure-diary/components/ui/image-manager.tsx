...
import { Upload, X, Image as ImageIcon, AlertCircle, Plus, Camera } from 'lucide-react';
import { EntityType, ImageInfo, parseImagesJson } from '@/lib/utils/imageUtils.client';
...
export function ImageManager({
  ...
  const handleFileSelect = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      // Check for errors
      const failedUploads = results.filter((result: any) => !result.success);
      if (failedUploads.length > 0) {
        setError(`${failedUploads.length} file(s) failed to upload: ${failedUploads.map((r: any) => r.error).join(', ')}`);
      }
    } catch (error) {
    ...
    <div className={className}>
      ...
      <div className="mb-6">
        <h2 className="text-2xl font-semibold text-base-content mb-2">
          {entityTypeNames[entityType]} Images
        </h2>
        <p className="text-base-content/70">
          Upload and manage images for this {entityTypeNames[entityType].toLowerCase()}. 
